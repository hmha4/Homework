...
void BulletPool::FireLaser(Character * start, Character * target, float duration, float coolTime)
{
	if (start == nullptr || target == nullptr) return;

	laser->Fire(start, target, duration, coolTime);
}
...
void BulletPool::Remove(Bullet * bullet)
{
